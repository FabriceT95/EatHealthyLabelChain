      ...
      .then(async json => {
        ...
        if (productVoteIsEndedArray.length > 0) {
          ...
          for (let i = 0; i < productVoteIsEndedArray.length; i++) {
            ...
            await worker.sendTransaction(rawTX).then(async (serializedTx) => {
              await web3.eth.sendSignedTransaction('0x' + serializedTx).on('receipt', async () => {
                if (productVoteIsEndedArray[i].status === 'IN_MODIFICATION' && productVoteIsEndedArray[i].for_votes >= productVoteIsEndedArray[i].against_votes) {
                  await fetch('http://192.168.42.219:8090/accepted_to_modified/' + productVoteIsEndedArray[i].product_code, {method: 'PUT'}).then(() => {
                ...
                setTimeout(async () => {
                  await fetch('http://192.168.42.219:8090/end_vote/' + productVoteIsEndedArray[i].product_code + '/' + productVoteIsEndedArray[i].for_votes + '/' + productVoteIsEndedArray[i].against_votes + '/' + productVoteIsEndedArray[i].end_date_timestamp + '', {method: 'PUT'}).then(() => {
                    console.log('SUCCESS');
                  });
                }, 1000);
